...
export default class APIRequest {
  ...
  passwordReset = async (data) => {
    const res = await this.instance.get(
      `/auth/password_reset_request?matriculation_number=${data}`,
    );
    return res;
  };
  ...
  confirmPasswordReset = async (token: string) => {
    const res = await this.instance.get(
      `/auth/password_reset_confirmation?password_reset_token=${token}`,
    );
    return res;
  };
  ...
  getLecturers = async (page: number) => {
    const res = await this.instance.get(
      "/years/api/lecturers/getAll?page=" + page,
    );
  ...
  getLevelCourses = async (level: number) => {
    const response = await this.instance.get(
      `/years/api/courses/getAll/${level}`,
    );
    const courses = response.data.data;
    console.log(response, courses);
    return courses;
